...
You can also select a range of lines by clicking the first line to select while holding down the Control key, then clicking the last line while still holding down the Control key.
...
The interface and messages logged in the logs are translated into the 5 languages supported by Jeedom (thanks to @mips for developing ga-translation). If you find any translation errors, please open a support ticket and, if possible, attach the corrected translation file (located in the plugin’s core/i18n directory).

The plugin documentation is available only in English (links to other languages redirect to the English translation). The translation is generated by a machine translator. However, the screenshots are not translated.
